...
const Planet = (props) => {
  ...
  <div className="card">
    ...
    <button type="Delete" onClick={props.onDelete}>Delete</button>
...
function App() {
  const [planets,setPlanets]= useState(planetsConstants);

  function onPlanetDelete(index){
    
    const tempPlanets=[...planets.slice(0,index),...planets.slice(index+1)]
  ...
  return <>
   <h1 className="yashvi">These are planets</h1>
   <div className="yashvi">
   </div>
   <div className="container">
   {planets.map(function(element,index){
    return <Planet name={element.name} desc={element.desc} diameter={element.diameter} moons={element.moons} image={element.url} onDelete={() => onPlanetDelete(index)} />
   })}
   </div>
